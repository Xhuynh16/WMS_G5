...
import { AREA_RIGHTS } from '../../shared/constants';@Component({
  selector: 'app-area',
  standalone: true,
  imports: [ShareModule],
...
export class AreaComponent {
  ...
  exportExcel() {
    return this._service.exportExcelArea(this.filter).subscribe((result: Blob) => {
  ...
  submitForm(): void {
    ...
    if (this.validateForm.valid) {
      if (this.edit) {
        this._service.updateArea(this.validateForm.getRawValue()).subscribe({
          next: (data) => {
            this.search();
          },
          error: (response) => {
        ...
        this._service.createArea(this.validateForm.getRawValue()).subscribe({
          next: (data) => {
            this.search();
          },
          error: (response) => {
